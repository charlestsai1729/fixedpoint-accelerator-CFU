    ...
    for (int out_y = 0; out_y < output_height; ++out_y) {
      ...
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          ...
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              for (; in_channel+4 <= input_depth; in_channel += 4) {
                ...
                acc = cfu_op0(1, input_val, filter_val);
              }
              // unsigned my_finish = perf_get_mcycle();
              // my_cycles += (my_finish - my_start);

              for (; in_channel < input_depth; in_channel += 1) {
          ...
          // cfu_op0(10, acc, output_multiplier[out_channel]);
          // acc = cfu_op0(11, output_shift[out_channel], 0);
